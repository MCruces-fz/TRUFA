...
//Loop over all events, data written to the root tree
{
	...
	TClonesArray*  RpcSaeta2Planes  = trackFinder->getRpcSaeta3Planes();
	...
	for(Int_t i=0; i< nbEvt; i++)
	{
		...
		if(prevsec>event->getEvtSecond()) {
		    cout<<"prevsec "<<prevsec<<" "<<event->getEvtSecond()<<endl;
		}
		prevsec = event->getEvtSecond();
		h_daq_active->Fill(timeEvt);
	    }
            h_daq_active_total->Fill(timeEvt);
		...
		TRpcSaeta* RpcSaeta  = (TRpcSaeta*)RpcSaeta2Planes->At(0);
		if(!track) {
		...
		for(Int_t tr=0;tr<ntracks;tr++) {
		    TRpcSaeta* RpcSaeta = (TRpcSaeta*)RpcSaeta2Planes->At(tr);
		    if(track) {
		...
		for(Int_t tr=0;tr<ntracks3;tr++) {
		    TRpcSaeta* RpcSaeta3Planes = (TRpcSaeta*)RpcSaeta3Planes->At(tr);
		    if(track) {
			if( std::find(usedhits1.begin(), usedhits1.end(), track->getInd(0)) == usedhits1.end() ) {
			    // not found. Then push_back
                            usedhits1.push_back(track->getInd(0));
...
void doStaffAnalysis(char* name){
    ...
    fill.setFileHitFinderPar("../pars2/"+day+"_CalPars.txt");
    fill.fillHistograms("/media/Datos2TB/tragaldabas/data/done/","../"+fName,"./",day+"_result_histos.root",1000000,0);
}
